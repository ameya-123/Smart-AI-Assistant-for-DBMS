  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem('userSkillLevel', data.skillLevel);
        navigate('/tutor'); 
      } else {
      ...
      <div className="max-w-md w-full space-y-8 bg-white/90 backdrop-blur-xl p-10 rounded-[2.5rem] shadow-2xl border border-white/50 relative z-10 transition-all duration-500">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSignIn}>
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input 
                  ...
                  className="appearance-none rounded-2xl block w-full pl-12 px-4 py-4 border border-gray-100 bg-gray-50/50 text-gray-900 font-medium focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all outline-none sm:text-sm" 
          ...
          <div className="pt-4">
            <button 
              type="submit" 
              disabled={loading} 
              className="w-full flex items-center justify-center py-4 px-6 border border-transparent text-sm font-black rounded-2xl text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 shadow-xl shadow-indigo-200 transition-all active:scale-[0.98] disabled:opacity-70"
            >
        ...
        <div className="text-center pt-2">
          <p className="text-sm text-gray-500 font-medium">
            Don't have an account? <Link to="/signup" className="font-bold text-indigo-600 hover:text-indigo-700 underline-offset-4 hover:underline">Sign up</Link>
          </p>
        </div>
      </div>
    </div>
